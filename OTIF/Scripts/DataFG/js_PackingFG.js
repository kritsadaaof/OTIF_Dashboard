...
$(document).ready(function () {
    ...
    $("#Key").html("รายการ Packing ");
    $.post(baseUrl + "Packing/GetDATAFG", { 
        BASE: "P1"
      }).done(function (data) {
...
function CalTime(A) { //หาวันที่และเวลา โดยปัจจุบัน หรือค่าที่ส่งเข้ามาเพื่อแสดงตาม Format
    var m = moment(A);
    //alert(m);
...
function CalTimeRDT(A) { //หาวันที่และเวลา โดยปัจจุบัน หรือค่าที่ส่งเข้ามาเพื่อส่งคำนวนต่อ
    var m = moment(A);
    //check parsed is correct
    if (m.isValid) {
        m.add({ Hours: 0, Minutes: A });
...
function CalMin(A,B,C) { //หาเวลาแผนผลิตเสร็จ เพื่อดึงวันที่ไปโชว์
...
function CalMinRDT(A, B, C) { //หาเวลาแผนผลิตเสร็จ เพื่อไปคำนวน
