...
$(document).ready(function () {
    ...
    $("#Save").click(function () {
        if ($("#Bar_Kam").val() != "" && $("#Qty").val() != "" && $("#User").val() != "" && $("#Actual_Speed").val() != "") {
            $.post(baseUrl + "ShopFloor/SaveSHF", {
                BAR_KAN: $("#Bar_Kam").val(),
                MACHINE: $("#Machin").val(),
                QTY: $("#Qty").val(),
        ...
        $.post(baseUrl + "ShopFloor/CheckBarSF", {
            ...
            if (data == "[]") {  
                ...
                if (pr[0]["SF_Remain_Qty"] == null) {
                    ...
                    $("#Pro_RemainHidden").html(pr[0]["QTY"]);
                    Pro_Remain = pr[0]["QTY"];
                }
                else {
                    $("#Pro_Remain").html(pr[0]["SF_Remain_Qty"]);
                    $("#Pro_RemainHidden").html(pr[0]["SF_Remain_Qty"]);
    ...
    $('#Qty').keyup(function () {
     /*   if (parseInt($('#Qty').val()) <= parseInt(Pro_Remain)) {
            $("#Pro_Remain").html(parseInt(Pro_Remain) - $('#Qty').val());
        }
        else if (parseInt($('#Qty').val()) > parseInt(Pro_Remain)) { //QTY มากกว่า Remain
            var nFrom = "bottom";
            var nAlign = "center";
            var nIcons = $(this).attr('data-icon');
            var nType = "danger";
            var nAnimIn = $(this).attr('data-animation-in');
            var nAnimOut = $(this).attr('data-animation-out');
            var mEss = "ไม่สามารถกรอก จำนวนมากกว่า Remain ได้";
            notify(nFrom, nAlign, nIcons, nType, nAnimIn, nAnimOut, mEss);
            $("#Qty").val("").focus();
            $("#Pro_Remain").html(Pro_Remain) //คืนค่า Remain
        }
        else {
            $("#Pro_Remain").html(Pro_Remain) //คืนค่า Remain
        } */

        $("#Pro_Remain").html($("#Pro_Traget").html() - $('#Qty').val());
      //  $("#Pro_Remain").html(parseInt(Pro_Remain) - $('#Qty').val());
    });
